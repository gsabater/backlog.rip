...
function filter(_filters, apps) {
  ...
  let re = /(?:^|[ ]):([a-zA-Z]+)/gm;
  let appName = '';
  let searchString = filters.string
      .toLowerCase()
      .replace(re, '')
      .trim();
  ...
  dance: for (let item of apps) {
      ...
      if (
          ...
          if (app.platforms === undefined || app.platforms === null || app.platforms === false) {
              continue;
          }
      ...
      if (filters.orderBy === 'hltb') {
          ...
          if (app.hltb.main === null || app.hltb.main === undefined || app.hltb.main === false) {
              counters.skip++;
              data.hidden.orderBy.hltb.push(appid);
              continue;
          }
